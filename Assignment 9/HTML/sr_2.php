...
<body>
  ...
  <div class="container">
    ...
    $sql = "SELECT * from Public where Interest like '%$search%'";
    ...
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        $highlighted_name = str_ireplace($search, "<mark>$search</mark>", $row['Interest']);
        echo '<td><a href="sd_2.php?uid=' . $row['User_id'] . '">' .$row['User_id'] . ' -- ' . $highlighted_name .'</a></td><br>';
      }
    } else {
